...
void Camera:: set(Point3D eye, Point3D look, Vector3D up) {
	this->eye.set(eye); // store the given eye position
...
void Camera::set(float eyeX, float eyeY, float eyeZ, float lookX, float lookY, 
	...
	Point3D eye = Point3D(eyeX, eyeY, eyeZ);
	Point3D look = Point3D(lookX, lookY, lookZ);	
...
void Camera::rotAxes(Vector3D& a, Vector3D& b, float angle) {
	...
	a.set(C * a.getX() + S * b.getX(), C * a.getY() + S * b.getY(), C * a.getZ() + S * b.getZ());
	b.set(-S * a.getX() + C * b.getX(), -S * a.getY() + C * b.getY(), -S * a.getZ() + C * b.getZ());
}

void Camera::setDefaultCamera() {
	setShape(45.0f, 1.0f, 0.1f, 200.0f);
